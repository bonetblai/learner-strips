...
NUM_CONSTANTS_1 = ('num_constants', 1)
CONSTANT_VAR_2 = ('constant_var', 2)
CONSTANT_VALUE_2 = ('constant_value', 2)
ACTION_1 = ('action', 1)
AFFECTS_2 = ('affects', 2)
...
def num_constants_1(n):
    return clingo.Function('num_constants', [clingo.Number(n)])

def constant_var_2(t ,c):
    return clingo.Function('constant_var', [clingo.Number(t), clingo.Number(c)])

def constant_value_2(t ,c):
    return clingo.Function('constant_value', [clingo.Number(t), clingo.Number(c)])

def constant_all_in_2(t, c):
    return clingo.Function('all_in', [clingo.Number(t), clingo.Number(c)])

def constant_all_out_2(t, c):
    return clingo.Function('all_out', [clingo.Number(t), clingo.Number(c)])
...
class STRIPSSchema:
    ...
    def add_constant(self, c):
        if c not in self._constants:
            self._constants[c] = dict(var=set(), value=set(), all_in=set(), all_out=set())

    def add_constant_var(self, t, c):
        self.add_vartype(t)
        self.add_constant(c)
        self._constants[c]['var'].add(t)

    def add_constant_value(self, t, c):
        self.add_vartype(t)
        self.add_constant(c)
        self._constants[c]['value'].add(t)

    def add_constant_all_in(self, t, c):
        self.add_vartype(t)
        self.add_constant(c)
        self._constants[c]['all_in'].add(t)

    def add_constant_all_out(self, t, c):
        self.add_vartype(t)
        self.add_constant(c)
        self._constants[c]['all_out'].add(t)
    ...
    def create_from_clingo(cls, symbols : List[clingo.Symbol]):
        ...
        for symbol in symbols:
            ...
            elif symbol.match(*CONSTANT_VAR_2):
                t, c = symbol.arguments
                schema.add_constant_var(t.number, c.number)
            elif symbol.match(*CONSTANT_VALUE_2):
                t, c = symbol.arguments
                schema.add_constant_value(t.number, c.number)
            elif symbol.match(*ALL_IN_2):
                t, c = symbol.arguments
                schema.add_constant_all_in(t.number, c.number)
            elif symbol.match(*ALL_OUT_2):
                t, c = symbol.arguments
                schema.add_constant_all_out(t.number, c.number)
    ...
    def get_string(self, val=False):
        ...
        if len(self._constants):
            ...
            for c in self._constants:
                ...
                out += f' C{c} ['
                out += f"var=({','.join([ 'T' + str(t) for t in self._constants[c]['var'] ])}), "
                out += f"value=({','.join([ 'T' + str(t) for t in self._constants[c]['value'] ])}), "
                out += f"all_in=({','.join([ 'T' + str(t) for t in self._constants[c]['all_in'] ])}), "
                out += f"all_out=({','.join([ 'T' + str(t) for t in self._constants[c]['all_out'] ])})"
                out += ']'
            out += '\n'
            ...
            for action in self._actions.keys():
                ...
                if action in self._appl[inst]:
                    ...
                    for (s1, args) in sorted(self._appl[inst][action]):
                        if inst not in self._next or action not in self._next[inst]: continue
                        if (s1, args) not in self._next[inst][action]: continue
    ...
    def get_schema(self):
        ...
        for c in self._constants:
            ...
            for t in self._constants[c]['var']:
                symbols.append(constant_var_2(t, c))
            for t in self._constants[c]['value']:
                symbols.append(constant_value_2(t, c))
            for t in self._constants[c]['all_in']:
                symbols.append(constant_all_in_2(t, c))
            for t in self._constants[c]['all_out']:
                symbols.append(constant_all_out_2(t, c))
        symbols.append(num_constants_1(len(self._constants)))
